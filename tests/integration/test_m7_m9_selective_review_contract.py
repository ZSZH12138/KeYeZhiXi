...
def test_pending_scores_wait_and_rejected_scores_cannot_enter_mastery() -> None:
    service = M8AssessmentService(_M8Repository(), object(), object())
    pending = service.finalize_scoring(
        _preparation(),
        [_result(flags=["teacher_review_required"])],
    )
    assert pending.requires_teacher_review() is True
    assert pending.has_rejected_score() is False

    rejected = pending.model_copy(deep=True)
    current = rejected.get_audit_record(pending.score_audit_records[0].audit_id)
    rejected.replace_audit_record(
        current.model_copy(
            update={
    ...
    try:
        ...
        assert error.code == "SCORE_REJECTED_PENDING_RESCORE"
    else:
        raise AssertionError("rejected scores must not enter mastery")
