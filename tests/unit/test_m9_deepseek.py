...
def test_sqlite_refuses_checksum_less_m9_audit_table_atomically(
    tmp_path: Path,
) -> None:
        ...
        connection.execute(
            ...
            CREATE TABLE m9_model_invocation_audits (
                ...
                provider_status TEXT NOT NULL CHECK (
                    provider_status IN (
                        'not_run', 'succeeded', 'failed', 'blocked'
                    )
                ),
    ...
    with connect_sqlite(database_path) as connection:
        with pytest.raises(
            RuntimeError,
            match="m9_model_invocation_audits schema is incompatible",
        ):
            migrate(connection)
        ledger_version = int(
            connection.execute(
    ...
    assert ledger_version == SCHEMA_VERSION
