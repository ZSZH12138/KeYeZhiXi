...
班级人数从当前活动成员关系动态读取，升级前旧班级兼容原有效授权，不再以固定配置作为当前班级人数。班级知识地图默认折叠，显示知识点名称、未做人数，并只用做过该知识点的学生计算平均掌握度和补学度；无人做过时两项均为 `0`。学生画像只显示本人做过的知识点。教师可用课程、班级和学生账号查询普通复核记录，也可让 DeepSeek 根据已做且掌握度最低的前 10 个知识点及课程原文生成教学建议。

### 2026-08-29 账户与开课治理

管理员登录后只管理教师、学生账户；教师和学生不能自行创建或注销账号。教师原课程/班级填写页新增“开课”，可命名课程和班级、建立独立班级容器，并在原班级页按学生账户名邀请或移出成员、查看人数和完整名单。新班级以活动成员关系作为访问和统计依据，名单变化会同步 M5/M9 与未做人数；旧班级继续兼容原授权。注销采用跨 M0、M4—M9 的物理清除，删除会话、个人记录和用户行，失败时保持冻结并允许管理员重试。
...
- 活动班级成员关系形成新班级动态名单，旧班级兼容原授权；画像和名单变化同步重建 M5/M9 快照，教师读取时还会按当前名单和活动知识版本修复陈旧快照；
- 三类账户隔离、管理员专用账户控制台、教师开课与班级成员治理，以及跨模块物理注销；
