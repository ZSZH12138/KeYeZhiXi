...
- 学生表单转换的 `AssessmentSubmission` 与教师逐题改判表单；前者以题目实例 ID
  映射标量答案，后者只允许在题目满分范围内改分并可附教师备注。
...
当前实现包含 `.env` / `app.json` 配置加载、`roles.csv` 校验与同步、
...
- 学生人数由当前课程班级的有效学生授权动态计算，并冻结到每次画像测评；不再依赖固定班级人数。
- 只有已完成的诊断测评和阶段评测进入学生历史、普通复核和建议复核。随心练习与订正提交后仅保留完成当前页面所需的临时状态。
- 订正满分后关闭对应错题；学生以后在画像测评中再次答错时重新开启，不把订正本身伪装成一次画像测评。
- 教师普通复核按课程、班级、学生账号进入；低置信度建议复核按同一学生、同一试卷聚合。逐题改判会幂等同步学生画像和班级快照。
- 班级页读写 M5/M9 持久化快照，并在读取时校正。知识地图默认折叠，显示知识点名称和未做人数；未做学生不进入平均掌握度或补学度分母。

## 账户与班级治理

- 管理员只创建和物理注销教师、学生账户；教师和学生没有账户管理权限。
- 外部账户名与内部伪匿名 `actor_id` 分离，账户名和密码只要求非空且不能全为空白。
- 物理注销不保留账户名黑名单；相同账户名可用于新的独立账户。
- 教师在原课程班级页创建带课程名称和班级名称的容器，并按学生账户名邀请或移出成员。
- 教师首页从数据库读取本人拥有的全部活动班级，界面显示名称，内部 ID 只负责稳定引用。
- 注销教师会在同一数据库事务中清除其全部班级、成员关系、课程文件、知识发布、题库、知识点、模型配置和学习数据，再删除教师账户；事务后的文件清理由受限待办队列重试。
