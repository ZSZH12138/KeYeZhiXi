...
> 当前版本：2026-08-29 账户与开课治理、新知识入库、确定性优先评分、动态班级画像与可溯源 RAG 架构
>
...
平台账户固定为学生、教师、管理员三类，类型与权限同时校验：管理员登录后只进入账户控制台，负责新增教师、学生账户和物理注销；教师、学生没有创建或注销账户的入口和服务端权限。`roles.csv` 仅保留系统管理员的恢复引导，不再批量创建教师、学生，避免已注销账户被种子重新生成。

教师原首页和班级页全部保留，并在原课程/班级入口增加“开课”。教师填写课程名和班级名后创建独立工作区，成为唯一班级负责人；随后按学生账户名邀请或移出学生。负责人关系与活动班级成员关系是新班级的权威授权，管理员没有开课、邀请、移出或查看班级业务的权限。

物理注销按“冻结登录 → 清除 M0/M4/M5/M6/M7/M8/M9 中该账号的个人记录 → 清理会话与 Django 个人投影 → 删除 User 行”执行。教师注销会归档其班级并清除班级内保存的模型密钥，但保留课程内容和历史班级容器；学生注销会级联移除班级成员关系。审计只保留不可逆 HMAC 指纹、账号类型和结果，不保留被删账户名。任一模块清除失败时，账号保持冻结并记录失败，管理员可以重试，不会把半完成删除伪装为成功。
...
新建班级的当前名单来自精确工作区下状态为活动且账号可用的学生成员关系；升级前没有负责人的旧班级继续读取启用、未撤销、在有效期内的学生授权，避免破坏已有课程。开始会改变画像的作答和教师改判时会冻结该名单的人数、校验和与时间，用动态人数覆盖旧的固定 `class_size`；邀请、移出、账号注销或画像变化都会增加名单版本并同步修复 M5/M9 快照，因此学生总数和每题未做人数无需教师手工修改。
...
| 模块 | 当前变化 | 保留能力 |
|---|---|---|
| M0 | 补充三类账户、管理员账户控制台、教师开课/成员治理、跨模块物理注销、多文件 UI、入库任务和学生答疑 | 原教师/学生页面、登录、会话、日志、outbox、成绩复核 |
...
| M5 | 消费新发布版本投影出的 Q 矩阵；按活动班级成员关系和权威掌握计数维护班级快照 | DINA/BKT 与状态版本化 |
...
- 管理员只管理账户；教师只能管理自己负责的活动班级；学生只能读取自己仍在班级名单中的活动版本。
- 物理注销要求再次输入完整账户名；底层删除语句使用固定表和参数化条件，失败时保持账号冻结并记录不含账户名的审计。

## 运行入口

- 教师知识文件：`/teacher/courses/<course_id>/knowledge/`
- 管理员账户控制台：`/account-admin/accounts/`
- 教师开课：`/teacher/classes/open/`
