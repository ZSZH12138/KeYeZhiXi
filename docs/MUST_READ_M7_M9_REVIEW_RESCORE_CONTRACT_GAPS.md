...
1. **PR1 只完成 B-05 的 M7/M9 侧防护，不能宣称 B-05 已全链路关闭。**
2. 当前 `reject` 能保存教师决定、保留被拒绝的原分、避免把它伪装成零分，并能让
   M9 不再生成看似权威的个人最近成绩；这些都是必要防护，但不能撤销提交阶段已经
   写入 M5 的影响。
3. 当前唯一可达的恢复路径仍是教师提交完整、守恒的 `override`。代码中没有一条
   完整、持久、可恢复的模型 `rescore` 路径。
4. 真正关闭 B-05 的首选方案是**延迟入账**：只要一次 attempt 的任一最新审计版本
   仍为 `pending` 或 `rejected_pending_rescore`，该 attempt 的分数就不得进入 M5、
   M6 或权威 M9 报表。不要优先选择事后“冲销”，因为 M5 聚合、M6 动作、学生反馈
   和已发布事件都可能已经被污染。
...
> `partially_mitigated_in_m7_m9; cross_module_blocker_open`

不得使用 `fixed`、`closed`、`reject 已闭环`、`已清除污染` 或等价表述。

## 2. 当前真实调用顺序与污染点

### 2.1 首次提交

当前 `AssessmentWorkflow.submit_assessment(...)` 的真实顺序是：

1. M8 `prepare_scoring(...)` 拆出客观题审计与 M7 主观评分任务；
2. Application 调 M2 取证，再逐项调用 M7 `score_subjective_answer(...)`；
3. M8 `finalize_scoring(...)` 生成并持久化 `ScoringResultBundle`；M7 的
   `review_flags` 在这里被映射为最新审计的 `pending` 或 `not_required`；
4. Application 在 `scoring_saved` 后无条件调用
   `M0.append_learning_events(scoring.learning_events)`；
5. Application 冻结当时的 learner/class 输入，然后调用 M5
   `update_state_with_frozen_policy(...)`；
6. Application 把评分和已经更新的 M5 状态交给 M6，生成后续教学动作；
7. Application 调 M7 生成学生反馈；
8. Application 把评分和 M5 状态交给 M9，生成教师分析；
9. M0 把 submit run 标为 `completed`。

对应代码入口与关键位置：

- `src/course_insight/application/assessment_workflow.py`：M7 评分约在
  `score_subjective_answer`，M8 汇总约在 `finalize_scoring`；紧接着是
  `append_learning_events`、M5、M6、M7 反馈和 M9 分析。
- `src/course_insight/modules/m8_assessment_scoring/service.py`：
  `finalize_scoring(...)` 根据 `review_flags`/置信度写 `pending`，但这不是入账门。
- `src/course_insight/contracts/assessment.py`：
  `ScoringResultBundle.requires_teacher_review()` 已能判断最新版本是否还有待复核项，
  但 Application 当前没有在 M5 前使用它。

污染发生在教师有机会操作之前：

- 分数相关 learning event 已追加；
- M5 的 learner mastery、misconception、class aggregate 和已处理审计水位已变化；
- M6 已基于该状态决定动作；
- M7 反馈和 M9 首份报告已基于该状态生成；
- submit 已成为当前复核入口所要求的 `completed` run。

### 2.2 教师复核

当前 `AssessmentWorkflow.review(...)` 的真实顺序是：

1. `AssessmentResults.completed_submit(...)` 强制要求先存在 completed submit；
2. M9 `record_teacher_review(...)` 保存 `confirm`、`override` 或 `reject` 决定；
3. M8 `apply_teacher_review(...)` 追加一个新审计版本；`reject` 会得到
   `review_status="rejected_pending_rescore"`，但仍保留原分，不会记零；
4. Application 追加教师复核事件；
5. 非 reject 才可能再次调用 M5；reject 分支直接复用 `base_run` 的 state；
6. M9 为 reject 构造不含有效个人最近成绩的 pending-rescore 报告。

第 5 步只是“不再追加一次污染”，不是撤销第 2.1 节已经发生的污染。尤其是
`assessment_workflow.py` 的 reject 分支把 `base_run`/`previous_state` 继续作为结果，
所以只检查 `state_version` 没有增加，无法证明 mastery、class aggregate 或审计水位
已经恢复。

### 2.3 当前为何没有真正的 rescore

以下约束同时存在：

- M0 的 `WorkflowOperation` 只有 `start | submit | review`，没有 `rescore`；
- M0 的 submit/review completed 校验要求已有 scoring checksum、state version 和
  report id，submit 还要求 feedback id，无法表达“评分已保存、等待教师、尚未入账”；
- review 只能从 completed submit 开始；
- `ScoreAuditRecord.scoring_method` 只有 `rule | local_model | teacher_override`；
- `validate_audit_history(...)` 要求审计版本大于 1 时必须为
  `teacher_override`，因此模型重评分不能合法追加成 v2/v3；
- M0 workflow 元数据刻意不保存业务 payload；原始主观答案也没有作为可恢复的
  rescore 输入保存在 M8 最终评分包中。

在这些约束没有被正式改造前，任何“再次调用 M7 然后覆盖原记录”的实现都会破坏
审计历史、幂等或隐私边界，不能作为 rescore。
...
> 本 PR 完成 M7/M9 范围内的安全止血和契约恢复；B-05 仍缺 Application/M0/M8 的
> 延迟入账与 rescore 协议，不能宣称全链路关闭。当前 reject 后只有教师完整 override
> 是可达恢复路径。
